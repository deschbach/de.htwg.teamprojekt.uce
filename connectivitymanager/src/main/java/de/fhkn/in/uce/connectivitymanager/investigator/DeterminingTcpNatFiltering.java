...
final class DeterminingTcpNatFiltering implements DeterminingNATFeatureRealization {
    ...
    private Socket createConnectedSocket(final InetSocketAddress address) throws IOException {
        ...
        logger.debug("connecting to address {} on local port {}", address, this.sourcePort); //$NON-NLS-1$
        socket.connect(address);
        return socket;
    }

    public static void main(String[] args) {
        final InetSocketAddress stunServerAddress = new InetSocketAddress("213.239.218.18", 3478);
